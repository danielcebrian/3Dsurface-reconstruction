function  [X,Y,BUTTON,SCALEMAT] = ginput22(varargin)
%ginput22   Graphical input from mouse with zoom, pan, plot and scaling.
%   
%   SYNTAX:
%                        XY = ginput22;
%                        XY = ginput22(DoScale);          true or false
%                        XY = ginput22(...,PlotOpt);      '.r' for example
%                        XY = ginput22(...,'KeepZoom');   vs. 'UnZoom'
%                        XY = ginput22(N,...);
%                        XY = ginput22(...);
%                     [X,Y] = ginput22(...);
%              [X,Y,BUTTON] = ginput22(...);
%     [X,Y,BUTTON,SCALEMAT] = ginput22(...);
%
%   INPUT:
...
%   DISCLAIMER:
%   ginput22.m is provided "as is" without warranty of any kind, under the
%   revised BSD license.
...
ZOOMIN    = UPARROW;%LEFTCLICK;             % ZOOM(2)    buttons
ZOOMRECT    = LEFTCLICK;             % ZOOM(2)    buttons
ZOOMRESET = ESCAPE;           % ZOOM RESET buttons
ZOOMOUT   = DOWNARROW;%DOUBLECLICK;           % ZOOM OUT   buttons
% Other parameters
...
if nargout>5
 error('CVARGAS:ginput22:tooManyOutputs',...
...
% Checks ginput version 1=ginput 2=ginput2:
if ~isempty(varargin) && ~isempty(varargin{1}) && ...
  (strcmp(varargin(1),'v1') || strcmp(varargin(1),'v2'))
 if strcmp(varargin(1),'v1')
    ginputVersion           = 1; % Forced unique, positive
 else
    ginputVersion           = 2; % Forced unique, positive
 end
 varargin(1) = [];                         % integer.
end
 ...
 if ~isempty(hi)
  ...
  if isempty(SCALEMAT) % interactively
   ...
   while true
    ...
    [I1x,I1y] = ginput22(1,false,'none','UnZoom');
    I1x       = round(I1x);
    I1y       = round(I1y);
   ...
   if ~isempty(answer) && isfloat(answer) && (length(answer)==2) && ...
    ...
    secondcorner = false;
    warning('CVARGAS:ginput22:incorrectGeoreference',...
   ...
   while secondcorner
    ...
    [I2x,I2y] = ginput22(1,false,'none','UnZoom');
    I2x       = round(I2x);
    I2y       = round(I2y);
    if ~ishandle(ha), return, end
  ...
  warning('CVARGAS:ginput22:noImageFound',...
...
while cont<N     % Principal loop
 ...
 try
  ...
  % GINPUT error:
  if YESERROR
     error('CVARGAS:ginput22:executionError',lasterr)
  else
   warning('CVARGAS:ginput22:executionError',lasterr)
 ...
 if ismember(button,ZOOMRECT) && ~outside
  % Dragging rectangle:
  undoPtrFig = gcf;
  setptr(undoPtrFig,'closedhand')
  rbbox
